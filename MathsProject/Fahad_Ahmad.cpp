...
x ufo[5];//these are the 5 ufos which now have Cy and Cv values
...
frwrd ahead[5] = { 1,1,1,1,1 }; //to check if going forward or not

bool ctimer = 0; //indicator of active timer
int val=0;
int counter=0, minutes=0; //time variables
bool start = 0;
int clr;
...
int max()	//calculating winner in forward direction
{
...
int min() //calculating winner in backward direction
{
	for (int z = 0; z < 5; z++)
	{	if(ahead[z].fr==0)
...
void displayufo(int cx, int cy, int r, int pos) 
{	
	...
	glColor3f(0, 0, 0);
	vprint(ufo[pos].Cx + 32, ufo[pos].Cy - 18, GLUT_BITMAP_8_BY_13, "0%d", pos + 1);
}
...
void display() {
	...
	for (int m = 0; m < 5; m++)
	{
		displayufo(ufo[m].Cx, ufo[m].Cy, r[m], m);
	}
...
void onKeyDown(unsigned char key, int x, int y)
{
	...
	if (key == 32) //When spacebar is pressed
		ctimer = !(ctimer);
...
void onSpecialKeyDown(int key, int x, int y)
{
	...
	if (key == GLUT_KEY_F1) //initialization if F1 is pressed
	{
		for (int s = 0; s < 5; s++)
		{
			ufo[s].Cx = -400;
...
void main(int argc, char *argv[]) {
	...
	glutCreateWindow("Homework submission by Fahad Ahmad");
